...
public final class BagGameTests {
    private static ServerPlayer player(GameTestHelper h) {
        return new net.minecraftforge.common.util.FakePlayer(h.getLevel(), new com.mojang.authlib.GameProfile(java.util.UUID.randomUUID(), "TorchBagTest"));
    }
    ...
    public static void storage(GameTestHelper h) {
        ...
        bag.setHoverName(Component.literal("Saved name"));
    ...
    public static void backpackFallback(GameTestHelper h) {
        ...
        ServerPlayer p = player(h);
        ItemStack bag = new ItemStack(TorchBagMod.item(BagTier.LEATHER));
        ItemStack backpack = new ItemStack(net.minecraft.core.registries.BuiltInRegistries.ITEM.get(new net.minecraft.resources.ResourceLocation("sophisticatedbackpacks", "backpack")));
        h.assertTrue(!backpack.isEmpty(), "Backpack registry item missing");
        var storage = backpack.getCapability(net.p3pp3rf1y.sophisticatedbackpacks.api.CapabilityBackpackWrapper.getCapabilityInstance()).orElseThrow(() -> new IllegalStateException("Missing backpack capability")).getInventoryHandler();
    ...
    public static void cancelledPlacementKeepsTorches(GameTestHelper h) {
        ...
        p.getInventory().setItem(p.getInventory().selected,bag);
        int[] cancelled = {0};
        java.util.function.Consumer<net.minecraftforge.event.level.BlockEvent.EntityPlaceEvent> deny = event -> {
